...
$id = $price = $title = $thumbnail = $content = $id_category = $id_xuatxu = '';
if (!empty($_POST)) {
	...
	if (isset($_POST['id_xuatxu'])) {
		$id_xuatxu = $_POST['id_xuatxu'];
	}
	if (!empty($title)) {
		$created_at = $updated_at = date('Y-m-d H:s:i');
		//Luu vao database
		if ($id == '') {
			$sql = 'insert into product(title, thumbnail, price, content, id_category, created_at, updated_at) values ("'.$title.'", "'.$thumbnail.'", "'.$price.'", "'.$content.'", "'.$id_category.'", "'.$id_xuatxu.'", "'.$created_at.'", "'.$updated_at.'")';
		} else {
			$sql = 'update product set title = "'.$title.'", updated_at = "'.$updated_at.'", thumbnail = "'.$thumbnail.'", price = "'.$price.'", content = "'.$content.'", id_category = "'.$id_category.'", id_xuatxu = "'.$id_xuatxu.'" where id = '.$id;
		}
		...
		<div class="panel panel-primary">
			...
			<div class="panel-body">
				<form method="post">
					...
					<div class="form-group">
					  <label for="price">Chọn Xuất Xứ:</label>
					  <select class="form-control" name="id_xuatxu" id="id_xuatxu">
					  	<option>-- Lụa chọn xuất xứ --</option>
							<?php
							$sql          = 'select * from xuatxu';
							$xuatxuList = executeResult($sql);

							foreach ($xuatxuList as $item) {
								if ($item['id'] == $id_xuatxu) {
									echo '<option selected value="'.$item['id'].'">'.$item['name'].'</option>';
								} else {
									echo '<option value="'.$item['id'].'">'.$item['name'].'</option>';
								}
